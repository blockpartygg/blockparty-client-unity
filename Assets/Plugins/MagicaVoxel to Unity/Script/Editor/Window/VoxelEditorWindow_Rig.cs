	...
	public partial class VoxelEditorWindow {
				...
				LayoutV(() => {
					...
					LayoutH(() => {
						...
						if (GUI.Button(GUIRect(100, 22), "+ Human Bones", EditorStyles.miniButtonMid)) {
							int id = EditorUtil.DialogComplex("Add Human Bones", "Which type of bones do you want?", "Major Bones", "Full Bones", "Cancel");
							if (id != 2) {
			...
			if (!string.IsNullOrEmpty(path)) {
				...
				if (!string.IsNullOrEmpty(path)) {
					try {
						...
						EditorVoxelCore.CreateFileForResult(new List<VoxelCore.Result>() { result }, shader, scale);
		...
		private void ImportRigData () {
			...
			if (!string.IsNullOrEmpty(path)) {
				path = EditorUtil.RelativePath(path);
				if (!string.IsNullOrEmpty(path)) {
					if (EditorUtil.Dialog("Warning", "Overlap rig data for current sub model?", "OK", "Cancel")) {
						try {
							string json = Util.Load(path);
		...
		private void DeleteBoneDialog (int index) {
			if (index != -1 && EditorUtil.Dialog("Warning", string.Format("Delete the bone and all it's child bones?"), "Delete", "Cancel")) {
				var bones = TheBones;
				if (bones == null) { return; }
		...
		private void MakeWeightIntoData () {
			...
			if (TheWeights == null || !Data || !Data.Rigs.ContainsKey(CurrentModelIndex) || CurrentModelIndex > Data.Voxels.Count) { return; }

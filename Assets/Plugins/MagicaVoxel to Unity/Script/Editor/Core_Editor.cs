namespace VoxeltoUnity {
	...
	public static class EditorVoxelCore {
		...
		public static void CreateFileForResult (List<VoxelCore.Result> resultList, Shader shader, float scale) {
			...
			for (int index = 0; index < resultList.Count; index++) {
				...
				// File
				string path = Util.CombinePaths(result.ExportRoot, result.ExportSubRoot, result.FileName + result.Extension);

				string parentPath = Util.FixPath(Util.CreateParent(path));
				...
				if (result.Extension == ".prefab") {
					...
					for (int modelIndex = 0; modelIndex < model.Meshs.Length; modelIndex++) {
						...
						for (int i = 0; i < uMesh.Count; i++) {
							...
							Util.Save(obj, objPath);
							VoxelPostprocessor.AddObj(objPath, texturePath);

							if (hasObjBefore) {
								AssetDatabase.ImportAsset(EditorUtil.RelativePath(objPath), ImportAssetOptions.ForceUpdate);
							}
		...
		private static Transform CreateModelFrom (
			...
			if (vTransform.Model != null) {
				...
				if (materialsMap.ContainsKey(texture)) {
					mat = materialsMap[texture];
				} else {
					mat = new Material(TheShader) { mainTexture = texture };

namespace VoxeltoUnity {
	...
	public class VoxelFile {
		...
		public static VoxelData GetVoxelData (byte[] voxelBytes, bool isVox) {
			return isVox ? GetVoxelDataFromVox(voxelBytes) : GetVoxelDataFromQb(voxelBytes);
		}



		public static byte[] GetVoxelByte (VoxelData data, bool isVox) {
			return isVox ? GetVoxFromVoxelData(data) : GetQbFromVoxelData(data);
		}
		...
		private static VoxelData GetVoxelDataFromVox (byte[] voxBytes) {
			...
			using (MemoryStream ms = new MemoryStream(voxBytes)) {
				using (BinaryReader br = new BinaryReader(ms)) {
					...
					if (data.Transforms.Count == 0) {
						data.Transforms.Add(0, new VoxelData.TransformData() {
							ChildID = 1,
							Hidden = false,
							LayerID = -1,
							Name = "",
						...
						data.Shapes.Clear();
						data.Shapes.Add(1, new VoxelData.ShapeData() {
		...
		private static byte[] GetVoxFromVoxelData (VoxelData data) {
			if (!data) { return null; }
			List<byte> voxByte = new List<byte>();
			byte[] mainChrunk = WriteMain(data);
		...
		private static byte[] WriteMain (VoxelData data) {
		...
		private static byte[] WriteSize (int[,,] voxels) {
			List<byte> bytes = new List<byte>();
			bytes.AddRange(GetBytes(voxels.GetLength(0)));
			bytes.AddRange(GetBytes(voxels.GetLength(2)));
			bytes.AddRange(GetBytes(voxels.GetLength(1)));

namespace VoxeltoUnity {
	...
	public class UnlimitiedMesh {
		...
		public UnlimitiedMesh () { }



		public UnlimitiedMesh (List<Vector3> vertices, List<Vector2> uvs, List<BoneWeight> boneWeights = null) {
			SetData(vertices, uvs, boneWeights);
		}



		public void SetData (List<Vector3> vertices, List<Vector2> uvs, List<BoneWeight> boneWeights) {

			int vCount = vertices.Count;
			int meshNum = vCount / MAX_VERTEX_COUNT + 1;

			if (meshNum < Meshs.Count) {
				Meshs.RemoveRange(meshNum, Meshs.Count - meshNum);
			} else if (meshNum > Meshs.Count) {
				int addNum = meshNum - Meshs.Count;
